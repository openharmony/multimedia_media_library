...
#define MLOG_TAG "PortraitContactInfoRepo"
...
PortraitContactInfoRepository::PortraitContactInfoRepository(const std::shared_ptr<MediaLibraryRdbStore> &rdbStore)
    : rdbStore_(rdbStore)
{
}
...
static int32_t UpdateContactInfoWithAccurateRefresh(const std::vector<int32_t> &albumIds,
    const std::string &contactInfo)
{
    ...
    CHECK_AND_RETURN_RET_LOG(ret == E_OK, E_INNER_FAIL, "UpdateContactInfo init failed");
 
    std::string placeholders;
    std::vector<ValueObject> bindArgs;
    bindArgs.push_back(ValueObject(contactInfo));
    ...
    std::string sql = "UPDATE " + ANALYSIS_ALBUM_TABLE + " SET " + CONTACT_INFO + " = ? WHERE " +
                      ALBUM_ID + " IN (" + placeholders + ")";
 
    ret = albumRefresh.ExecuteSql(sql, bindArgs, AccurateRefresh::RdbOperation::RDB_OPERATION_UPDATE);
    CHECK_AND_RETURN_RET_LOG(ret == E_OK, E_INNER_FAIL, "UpdateContactInfo execute sql failed");
 
    albumRefresh.Notify();
    return E_OK;
}
 
bool PortraitContactInfoRepository::Exists(const std::string &albumId) const
{
...
int32_t PortraitContactInfoRepository::UpdateContactInfo(const std::string &albumId,
    const std::string &contactInfo) const
{
    MEDIA_INFO_LOG("UpdateContactInfo start");
    std::vector<int32_t> portraitAlbumIdsInt;
    ...
    CHECK_AND_RETURN_RET_LOG(UpdateContactInfoWithAccurateRefresh(portraitAlbumIdsInt, contactInfo) == E_OK,
        E_INNER_FAIL, "Failed to update contact_info for portrait album, albumId: %{public}s", albumId.c_str());
