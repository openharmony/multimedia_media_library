    ...
    int32_t version, int32_t upgradedIndex)
{
    ...
    std::string sql =
        "UPDATE " + VISION_VIDEO_FACE_TABLE + " SET " + TAG_ID + " = -1 WHERE " + TAG_ID + " IN ( SELECT DISTINCT " +
...
static int32_t AsyncUpgradeFromAllVersionFirstPart(RdbStore& rdbStore)
{
    ...
    if (prefs != nullptr) {
        ...
        if (detailTimeFixed == NEED_FIXED) {
            errCode = MediaLibraryRdbStore::UpdateDateTakenToMillionSecond(rdbStore);
            errCode = MediaLibraryRdbStore::UpdateDateTakenIndex(rdbStore);
    ...
    errCode = MediaLibraryRdbStore::AddIndexForFileIdAsync(rdbStore);
    MEDIA_INFO_LOG("End VERSION_ADD_INDEX_FOR_FILEID");

    MEDIA_INFO_LOG("Start VERSION_UPDATE_INDEX_FOR_COVER");
    errCode = MediaLibraryRdbStore::UpdateIndexForCover(rdbStore);
    ...
    if (prefs != nullptr) {
        ...
        if (thumbnailVisibleFixed == NEED_FIXED) {
            errCode = MediaLibraryRdbStore::UpdateThumbnailVisibleAndIdx(rdbStore);
    ...
    errCode = MediaLibraryRdbStore::UpdateDateTakenAndDetailTime(rdbStore);
    MEDIA_INFO_LOG("End VERSION_UPDATE_DATETAKEN_AND_DETAILTIME");

    MEDIA_INFO_LOG("Start VERSION_ADD_READY_COUNT_INDEX");
    errCode = MediaLibraryRdbStore::AddReadyCountIndex(rdbStore);
...
static int32_t AsyncUpgradeFromAllVersionSecondPart(RdbStore& rdbStore)
{
    ...
    ret = MediaLibraryRdbStore::AddCloudEnhancementAlbumIndex(rdbStore);
    MEDIA_INFO_LOG("End VERSION_ADD_CLOUD_ENHANCEMENT_ALBUM_INDEX");

    MEDIA_INFO_LOG("Start VERSION_UPDATE_PHOTOS_DATE_AND_IDX");
    ret = PhotoDayMonthYearOperation::UpdatePhotosDateAndIdx(rdbStore);
    ...
    ret = MediaLibraryRdbStore::UpdateLocationDefaultNull(rdbStore);
    ...
    ret = MediaLibraryRdbStore::UpdateMediaTypeAndThumbnailReadyIdx(rdbStore);
    ...
    ret = MediaLibraryRdbStore::UpdateLocationKnowledgeIdx(rdbStore);
    MEDIA_INFO_LOG("End VERSION_UPDATE_LOCATION_KNOWLEDGE_INDEX");

    MEDIA_INFO_LOG("Start VERSION_ADD_ALBUM_SUBTYPE_AND_NAME_INDEX");
    ret = MediaLibraryRdbStore::AddAlbumSubtypeAndNameIdx(rdbStore);
