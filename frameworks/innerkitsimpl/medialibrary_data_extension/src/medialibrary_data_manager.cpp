...
vector<string> MediaLibraryDataManager::ReadError()
{
    ...
    resultSet->GetColumnIndex(MEDIA_DATA_ERROR, columnIndex);
    ...
    while (resultSet->GoToNextRow() == NativeRdb::E_OK) {
        str.clear();
        resultSet->GetString(columnIndex, str);
        errList.push_back(str);
    }

    return errList;
}

int32_t MediaLibraryDataManager::RecordError(const string &err)
{
    auto rdbStore = MediaLibraryUnistoreManager::GetInstance().GetRdbStoreRaw();
    if (rdbStore == nullptr) {
        MEDIA_ERR_LOG("rdbStore is nullptr");
...
int32_t MediaLibraryDataManager::DeleteError(const string &err)
{
    auto rdbStore = MediaLibraryUnistoreManager::GetInstance().GetRdbStoreRaw();
    if (rdbStore == nullptr) {
        MEDIA_ERR_LOG("rdbStore is nullptr");
