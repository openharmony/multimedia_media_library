...
void MediaLibraryRdbStore::Init()
{
    ...
    if (rdbStore_ == nullptr) {
        MEDIA_WARNING_LOG("GetRdbStore is failed ");
        return;
    }
    ...
    if (rdbDataCallBack.GetDistributedTables()) {
        ...
        MEDIA_INFO_LOG("ret = %{private}d", ret);
    }

    if (!SubscribeRdbStoreObserver()) {
        MEDIA_WARNING_LOG("subscribe rdb observer err");
        return;
    }
...
{
    if (rdbStore_ == nullptr) {
        MEDIA_WARNING_LOG("SubscribeRdbStoreObserver rdbStore is null");
        return false;
    }
    ...
    MEDIA_INFO_LOG("Subscribe ret = %d", ret);
...
{
    if (rdbStore_ == nullptr) {
        MEDIA_WARNING_LOG("UnSubscribeRdbStoreObserver rdbStore is null");
        return false;
    }
    ...
    MEDIA_INFO_LOG("UnSubscribe ret = %d", ret);
    if (ret) {
...
int32_t MediaLibraryRdbStore::Delete(MediaLibraryCommand &cmd, int32_t &rowId)
{
    ...
    if (rdbStore_ == nullptr) {
        MEDIA_WARNING_LOG("Pointer rdbStore_ is nullptr. Maybe it didn't init successfully.");
        return DATA_ABILITY_FAIL;
    }

    if (cmd.GetAbsRdbPredicates() == nullptr) {
        MEDIA_WARNING_LOG("AbsRdbPredicates in input param(cmd) is nullptr.");
        return DATA_ABILITY_FAIL;
    }
    ...
    if (ret != NativeRdb::E_OK) {
        MEDIA_WARNING_LOG("rdbStore_->Delete failed, ret = %{public}d", ret);
        return DATA_ABILITY_FAIL;
    }
...
int32_t MediaLibraryRdbStore::Update(MediaLibraryCommand &cmd, int32_t &rowId)
{
    ...
    int32_t ret = rdbStore_->Update(rowId, cmd.GetTableName(), cmd.GetValueBucket(),
                                    ...
                                    cmd.GetAbsRdbPredicates()->GetWhereArgs());
    if (ret != NativeRdb::E_OK) {
        MEDIA_WARNING_LOG("rdbStore_->Update failed, ret = %{public}d", ret);
        return DATA_ABILITY_FAIL;
    }
...
int32_t MediaLibraryRdbStore::ExecuteSql(const std::string &sql)
{
    ...
    if (ret != NativeRdb::E_OK) {
        MEDIA_WARNING_LOG("rdbStore_->ExecuteSql failed, ret = %{public}d", ret);
        return DATA_ABILITY_FAIL;
    }
    return ret;
}
...
int32_t MediaLibraryDataCallBack::PrepareDir(RdbStore &store)
{
    ...
    for (auto dirValuesBucket : dirValuesBuckets) {
        if (InsertDirValues(dirValuesBucket, store) != NativeRdb::E_OK) {
            MEDIA_WARNING_LOG("PrepareDir failed");
...
int32_t MediaLibraryDataCallBack::InsertDirValues(DirValuesBucket dirValuesBucket, RdbStore &store)
{
...
int32_t MediaLibraryDataCallBack::PrepareSmartAlbum(RdbStore &store)
{
    ...
    for (auto smartAlbum : smartAlbumValuesBuckets) {
        if (InsertSmartAlbumValues(smartAlbum, store) != NativeRdb::E_OK) {
            MEDIA_WARNING_LOG("Prepare smartAlbum failed");
            return NativeRdb::E_ERROR;
        }
    }
    return NativeRdb::E_OK;
...
int32_t MediaLibraryDataCallBack::InsertSmartAlbumValues(SmartAlbumValuesBucket smartAlbum, RdbStore &store)
{
    ValuesBucket valuesBucket;
...
int32_t MediaLibraryDataCallBack::OnUpgrade(RdbStore &store, int32_t oldVersion, int32_t newVersion)
{
    ...
    if (result != NativeRdb::E_OK) {
        MEDIA_INFO_LOG("Upgrade rdb error %{private}d", result);
    }
#endif
    return NativeRdb::E_OK;
}
