...
static string GetGroupPhotoAlbumSql()
{
    ...
    string fullSql = "SELECT " + GROUP_PHOTO_TAG + ", " + PhotoAlbumColumns::ALBUM_ID + ", " +
        ...
        leftJoinAnalysisAlbum + " WHERE AA." + IS_REMOVED + " IS NOT " + to_string(ALBUM_IS_REMOVED) +
        " GROUP BY " + GROUP_PHOTO_TAG + ";";
    return fullSql;
}
...
std::shared_ptr<NativeRdb::ResultSet> MediaLibraryAlbumOperations::QueryGroupPhotoAlbum(MediaLibraryCommand &cmd,
    const std::vector<std::string> &columns)
{
...
int32_t UpdateMergeGroupAlbumsInfo(const vector<MergeAlbumInfo> &mergeAlbumInfo)
{
    auto uniStore = MediaLibraryUnistoreManager::GetInstance().GetRdbStore();
    if (uniStore == nullptr) {
    ...
    while (resultSet->GoToNextRow() == NativeRdb::E_OK) {
        ...
        if (GetIntValueFromResultSet(resultSet, ALBUM_ID, info.albumId) != E_OK ||
            ...
            MEDIA_ERR_LOG("UpdateMergeGroupAlbumsInfo db fail");
            return E_HAS_DB_ERROR;
        }
        ...
        if (reorderedTagId.empty()) {
            ...
            if (GetMergeAlbumCoverUri(newInfo, info, it->second) != E_OK) {
                return E_HAS_DB_ERROR;
            }
...
int32_t MergeAlbum(const ValuesBucket &values)
{
    ...
    err = UpdateMergeGroupAlbumsInfo(mergeAlbumInfo);
    if (err != E_OK) {
