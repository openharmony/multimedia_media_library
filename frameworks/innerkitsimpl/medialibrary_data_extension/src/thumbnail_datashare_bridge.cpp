...
std::shared_ptr<ResultSetBridge> ThumbnailDataShareBridge::Create(
    const std::shared_ptr<SingleKvStore> &singleKvStorePtr,
    const std::string &thumbnailkey)
{
    if (singleKvStorePtr == nullptr) {
        MEDIA_ERR_LOG("param error, singleKvStorePtr nullptr");
        return nullptr;
    }

    return std::shared_ptr<ResultSetBridge>(
        new (std::nothrow) ThumbnailDataShareBridge(singleKvStorePtr, thumbnailkey));
}
} // namespace Media
} // namespace OHOS