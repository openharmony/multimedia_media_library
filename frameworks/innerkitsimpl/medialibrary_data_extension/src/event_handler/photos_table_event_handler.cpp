...
#include "photos_table_event_handler.h"
 ...
 */
int32_t PhotosTableEventHandler::OnCreate(std::shared_ptr<MediaLibraryRdbStore> store)
{
 ...
 * @param store rdb store
 */
int32_t PhotosTableEventHandler::OnUpgrade(std::shared_ptr<MediaLibraryRdbStore> store, int oldVersion, int newVersion)
{
    ...
    MEDIA_INFO_LOG(
        "end upgrade %{public}s table, cost %{public}" PRId64 "ms.", PhotoColumn::PHOTOS_TABLE.c_str(), costTime);
    return ret;
}

int32_t PhotosTableEventHandler::AddWriterCodeColumn(
    std::shared_ptr<MediaLibraryRdbStore> store, int32_t oldVersion, int32_t newVersion)
{
    ...
    conn = !this->dbUpgradeUtils_.IsColumnExists(store, PhotoColumn::PHOTOS_TABLE, PhotoColumn::WRITER_CODE);
    CHECK_AND_RETURN_RET_WARN_LOG(conn,
        NativeRdb::E_OK,
        "column %{public}s already exists in table %{public}s",
        PhotoColumn::WRITER_CODE.c_str(),
        PhotoColumn::PHOTOS_TABLE.c_str());
    int64_t startTime = MediaFileUtils::UTCTimeMilliSeconds();
...
int32_t PhotosTableEventHandler::UpgradeTriggerForWriterCode(
    std::shared_ptr<MediaLibraryRdbStore> store, int32_t oldVersion, int32_t newVersion)
{
    bool conn = oldVersion < VERSION_ADD_WRITER_CODE_COLUMN;
    CHECK_AND_RETURN_RET(conn, NativeRdb::E_OK);
    ...
    const std::vector<std::string> sqls = {
        this->SQL_DROP_PHOTOS_FDIRTY_TRIGGER,
        this->SQL_CREATE_PHOTOS_FDIRTY_TRIGGER,
        this->SQL_DROP_PHOTOS_MDIRTY_TRIGGER,
        this->SQL_CREATE_PHOTOS_MDIRTY_TRIGGER,
        this->SQL_DROP_PHOTOS_DELETE_TRIGGER,
        this->SQL_CREATE_PHOTOS_DELETE_TRIGGER,
    };
    int32_t ret = NativeRdb::E_OK;
    for (auto &sql : sqls) {
