...
{
    MEDIA_INFO_LOG("zyc Stop rdbStore Enter");
    if (rdbStorePtr_) {
        rdbStorePtr_->Stop();
    }
...
{
    MEDIA_INFO_LOG("zyc rdbStorePtr_=%{public}p", rdbStorePtr_.get());
    if (rdbStorePtr_ != nullptr && rdbStorePtr_->CheckRdbStore()) {
        return rdbStorePtr_;
    }
    MEDIA_ERR_LOG("zyc MediaLibraryRdbStore or rdbStore is nullptr");
    return nullptr;
}
} // namespace Media
} // namespace OHOS
