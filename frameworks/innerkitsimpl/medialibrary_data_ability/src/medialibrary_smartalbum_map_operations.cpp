...
namespace OHOS
{
    namespace Media
    {
                                    ...
                                    const MediaLibrarySmartAlbumMapDb &smartAlbumMapDbOprn)
        {
            ...
            return const_cast<MediaLibrarySmartAlbumMapDb &>(smartAlbumMapDbOprn).InsertSmartAlbumMapInfo(values, rdbStore);
        }
        ...
        //         int32_t deleteResult = rdbStore->Delete(deletedRows, MEDIALIBRARY_TABLE, "data LIKE ? OR data = ?", whereArgs);
        ...
        //             "UPDATE MEDIALIBRARY_DATA SET path = replace(path, '" + albumPath + "/' , '" + newAlbumPath + "/'), " + "relative_path = replace(relative_path, '" + albumPath + "', '" + newAlbumPath + "'), " + "album_name = replace(album_name, '" + albumPath.substr(slashIndex + 1) + "', '" + albumNewName + "')" + "where data LIKE '" + albumPath + "/%'";
        ...
        //         int32_t deleteResult = rdbStore->Delete(deletedRows, MEDIALIBRARY_TABLE, "data LIKE ?", whereArgs);
        //         if (deleteResult != E_OK)
        //         {
        //             MEDIA_ERR_LOG("Delete rows failed");
        //         }
