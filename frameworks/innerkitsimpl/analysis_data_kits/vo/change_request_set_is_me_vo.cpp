...
#define MLOG_TAG "MediaChangeRequestSetIsMeVo"
...
using namespace std;
bool ChangeRequestSetIsMeReqBody::Unmarshalling(MessageParcel &parcel)
{
    bool status = parcel.ReadString(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    status = parcel.ReadInt32(this->isMe);
    CHECK_AND_RETURN_RET(status, status);
...
bool ChangeRequestSetIsMeReqBody::Marshalling(MessageParcel &parcel) const
{
