...
#define MLOG_TAG "MediaChangeRequestDismissVo"
...
using namespace std;
bool ChangeRequesDismissReqBody::Unmarshalling(MessageParcel &parcel)
{
    bool status = parcel.ReadString(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    status = parcel.ReadInt32(this->isRemoved);
    CHECK_AND_RETURN_RET(status, status);
...
bool ChangeRequesDismissReqBody::Marshalling(MessageParcel &parcel) const
{
    bool status = parcel.WriteString(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    status = parcel.WriteInt32(this->isRemoved);
    CHECK_AND_RETURN_RET(status, status);
