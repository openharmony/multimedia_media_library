...
HWTEST_F(MediaLibraryThumbnailUtilsTest, LoadAudioFileInfo_test_001, TestSize.Level0)
{
    ...
    Size desiredsize;
    uint32_t errCode;
    auto res = ThumbnailUits::LoadAudioFileInfo(avMetadataHelper, data, desiredsize, errCode);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, LoadVideoFrame_test_001, TestSize.Level0)
{
    ...
    int64_t timeStamp = 0;
    data.path = "test_path";
    auto res = ThumbnailUits::LoadVideoFrame(data, desiredsize, timeStamp);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, GenTargetPixelmap_test_001, TestSize.Level0)
{
    ThumbnailData data;
    Size desiredsize;
    auto res = ThumbnailUits::GenTargetPixelmap(data, desiredsize);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, SaveAfterPacking_test_001, TestSize.Level0)
{
    ...
    Size desiredsize;
    auto res = ThumbnailUits::SaveAfterPacking(data, isSourceEx, tempOutputPath);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, QueryLcdCount_test_001, TestSize.Level0)
{
    ...
    int err = 0;
    opts.store = nullptr;
    auto res = ThumbnailUits::QueryLcdCount(opts, outLcdCount, err);
    EXPECT_EQ(res, false);
    opts.store = ThumbnailService::GetInstance()->rdbStorePtr_;
    res = ThumbnailUits::QueryLcdCount(opts, outLcdCount, err);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, QueryLcdCountByTime_test_001, TestSize.Level0)
{
    ...
    int err = 0;
    opts.store = nullptr;
    auto res = ThumbnailUits::QueryLcdCountByTime(time, before, opts, outLcdCount, err);
    EXPECT_EQ(res, false);
    opts.store = ThumbnailService::GetInstance()->rdbStorePtr_;
    res = ThumbnailUits::QueryLcdCountByTime(time, before, opts, outLcdCount, err);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, QueryLocalNoLcdInfos_test_001, TestSize.Level0)
{
    ...
    int err = 0;
    opts.store = nullptr;
    auto res = ThumbnailUits::QueryLocalNoLcdInfos(opts, infos, err);
    EXPECT_EQ(res, false);
    opts.store = ThumbnailService::GetInstance()->rdbStorePtr_;
    res = ThumbnailUits::QueryLocalNoLcdInfos(opts, infos, err);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, QueryNoHighlightPath_test_001, TestSize.Level0)
{
    ...
    int err = 0;
    opts.store = nullptr;
    auto res = ThumbnailUits::QueryNoHighlightPath(opts, data, err);
    EXPECT_EQ(res, false);
    opts.store = ThumbnailService::GetInstance()->rdbStorePtr_;
    res = ThumbnailUits::QueryNoHighlightPath(opts, data, err);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, QueryNoHighlightInfos_test_001, TestSize.Level0)
{
    ...
    int err = 0;
    opts.store = nullptr;
    auto res = ThumbnailUits::QueryNoHighlightInfos(opts, infos, err);
    EXPECT_EQ(res, false);
    opts.store = ThumbnailService::GetInstance()->rdbStorePtr_;
    res = ThumbnailUits::QueryNoHighlightInfos(opts, infos, err);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, GetHighlightTracks_test_001, TestSize.Level0)
{
    ...
    int err = 0;
    opts.store = nullptr;
    auto res = ThumbnailUits::GetHighlightTracks(opts, trackInfos, err);
    EXPECT_EQ(res, false);
    opts.store = ThumbnailService::GetInstance()->rdbStorePtr_;
    res = ThumbnailUits::GetHighlightTracks(opts, trackInfos, err);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, UpdateHighlightInfo_test_001, TestSize.Level0)
{
    ...
    int err = 0;
    opts.store = nullptr;
    auto res = ThumbnailUits::UpdateHighlightInfo(opts, data, err);
    EXPECT_EQ(res, false);
    opts.store = ThumbnailService::GetInstance()->rdbStorePtr_;
    res = ThumbnailUits::UpdateHighlightInfo(opts, data, err);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, UpdateVisitTime_test_001, TestSize.Level0)
{
    ...
    int err = 0;
    opts.store = nullptr;
    auto res = ThumbnailUits::UpdateVisitTime(opts, data, err);
    EXPECT_EQ(res, false);
    opts.store = ThumbnailService::GetInstance()->rdbStorePtr_;
    res = ThumbnailUits::UpdateVisitTime(opts, data, err);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, CheckDateTaken_test_001, TestSize.Level0)
{
    ...
    data.id = "123";
    opts.store = nullptr;
    auto res = ThumbnailUits::CheckDateTaken(opts, data);
    EXPECT_EQ(res, false);
    opts.store = ThumbnailService::GetInstance()->rdbStorePtr_;
    res = ThumbnailUits::CheckDateTaken(opts, data);
    EXPECT_EQ(res, false);
}
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, GetLocalThumbSize_test_001, TestSize.Level0)
{
    ...
    auto res = ThumbnailUits::GetLocalThumbSize(data, type, size);
    EXPECT_EQ(res, false);
    ThumbnailType type = ThumbnailType::THUMB_ASTC;
    res = ThumbnailUits::GetLocalThumbSize(data, type, size);
    EXPECT_EQ(res, false);
...
HWTEST_F(MediaLibraryThumbnailUtilsTest, ConvertStrToInt32_test_001, TestSize.Level0)
{
    string str = "123";
    int32_t ret;
    auto res = ThumbnailUits::ConvertStrToInt32(str, ret);
    EXPECT_EQ(res, true);
    str = "test";
    res = ThumbnailUits::ConvertStrToInt32(str, ret);
    EXPECT_EQ(res, false);
    str = "-100000000000000";
    res = ThumbnailUits::ConvertStrToInt32(str, ret);
    EXPECT_EQ(res, false);
