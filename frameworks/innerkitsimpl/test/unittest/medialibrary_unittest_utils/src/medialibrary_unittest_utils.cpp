...
void MediaLibraryUnitTestUtils::CleanTestFiles()
{
    ...
    NativeRdb::AbsRdbPredicates predicates(CONST_MEDIALIBRARY_TABLE);
    predicates.GreaterThan(CONST_MEDIA_DATA_DB_ID, to_string(0));
...
void MediaLibraryUnitTestUtils::CleanBundlePermission()
{
    std::lock_guard<std::mutex> lock(Mutex_);
    auto rdbStore = MediaLibraryDataManager::GetInstance()->rdbStore_;
    NativeRdb::AbsRdbPredicates predicates(CONST_BUNDLE_PERMISSION_TABLE);
    predicates.GreaterThan(CONST_MEDIA_DATA_DB_ID, to_string(0));
    int32_t deletedRows = -1;
    auto ret = rdbStore->Delete(deletedRows, predicates);
...
bool MediaLibraryUnitTestUtils::GetFileAsset(const int fileId, shared_ptr<FileAsset> &fileAsset)
{
    ...
    string selections = string(CONST_MEDIA_DATA_DB_ID) + " = " + to_string(fileId);
    ...
    shared_ptr<FileAsset> &albumAsset)
{
    ...
    Uri createAlbumUri(MEDIALIBRARY_DATA_URI + "/" + CONST_MEDIA_ALBUMOPRN + "/" + CONST_MEDIA_ALBUMOPRN_CREATEALBUM);
    string dirPath;
    ...
    shared_ptr<FileAsset> &fileAsset)
{
    ...
    Uri createAssetUri(MEDIALIBRARY_DATA_URI + "/" + CONST_MEDIA_FILEOPRN + "/" + CONST_MEDIA_FILEOPRN_CREATEASSET);
    DataShareValuesBucket valuesBucket;
...
bool MediaLibraryUnitTestUtils::DeleteDir(const string &path, const string &dirId)
{
    ...
    auto rdbStore = MediaLibraryDataManager::GetInstance()->rdbStore_;
    NativeRdb::AbsRdbPredicates predicates(CONST_MEDIALIBRARY_TABLE);
    predicates.EqualTo(CONST_MEDIA_DATA_DB_ID, dirId)->Or()->EqualTo(CONST_MEDIA_DATA_DB_PARENT_ID, dirId);
    int32_t deletedRows = -1;
    auto ret = rdbStore->Delete(deletedRows, predicates);
    return ret == 0;
}
...
{
    std::lock_guard<std::mutex> lock(Mutex_);
    Uri addPermission(string(CONST_MEDIALIBRARY_BUNDLEPERM_URI) + "/" + CONST_BUNDLE_PERMISSION_INSERT);
    DataShareValuesBucket values;
    values.Put(CONST_PERMISSION_FILE_ID, fileId);
    values.Put(CONST_PERMISSION_BUNDLE_NAME, bundleName);
    values.Put(CONST_PERMISSION_MODE, mode);
    values.Put(CONST_PERMISSION_TABLE_TYPE, tableType);
