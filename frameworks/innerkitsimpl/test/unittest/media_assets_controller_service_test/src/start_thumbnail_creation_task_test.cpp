...
#include "start_thumbnail_creation_task_test.h"
...
void StartThumbnailCreationTaskTest::SetUpTestCase(void)
{
...
void StartThumbnailCreationTaskTest::TearDownTestCase(void)
{
    ClearTable(PhotoColumn::PHOTOS_TABLE);
...
void StartThumbnailCreationTaskTest::SetUp()
{
    ClearTable(PhotoColumn::PHOTOS_TABLE);
    std::this_thread::sleep_for(std::chrono::seconds(SLEEP_SECONDS));
...
void StartThumbnailCreationTaskTest::TearDown(void)
{
...
static int32_t QueryThumbnailReadyByDisplayName(const string &displayName)
{
    vector<string> columns;
    RdbPredicates rdbPredicates(PhotoColumn::PHOTOS_TABLE);
    rdbPredicates.EqualTo(MediaColumn::MEDIA_NAME, displayName);
...
HWTEST_F(StartThumbnailCreationTaskTest, StartThumbnailCreationTask_Test_001, TestSize.Level0)
{
