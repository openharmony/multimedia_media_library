...
void SetEditDataTest::SetUpTestCase(void)
{
...
void SetEditDataTest::TearDownTestCase(void)
{
    ClearTable(PhotoColumn::PHOTOS_TABLE);
...
void SetEditDataTest::SetUp()
{
    MEDIA_INFO_LOG("SetUp");
}

void SetEditDataTest::TearDown(void)
{
...
HWTEST_F(SetEditDataTest, SetEditDataTest_Test_001, TestSize.Level0)
{
    ...
    service->AssetChangeSetEditData(data, reply);
    ...
    EXPECT_NE(resSet, nullptr);
    EXPECT_EQ(resSet->GoToNextRow(), NativeRdb::E_OK);
    std::string newFilePath = GetStringVal(MediaColumn::MEDIA_FILE_PATH, resSet);
    EXPECT_EQ(newFilePath, filePath);
}
