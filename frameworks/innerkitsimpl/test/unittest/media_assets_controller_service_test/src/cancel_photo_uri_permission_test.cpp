...
#include "cancel_photo_uri_permission_test.h"
...
void CancelUriPermissionTest::TearDownTestCase(void)
{
    ClearTable(PhotoColumn::PHOTOS_TABLE);
    ClearTable(AppUriPermissionColumn::APP_URI_PERMISSION_TABLE);
    ClearTable(AppUriSensitiveColumn::APP_URI_SENSITIVE_TABLE);
...
void CancelUriPermissionTest::SetUp()
{
    ClearTable(PhotoColumn::PHOTOS_TABLE);
    ClearTable(AppUriPermissionColumn::APP_URI_PERMISSION_TABLE);
    ClearTable(AppUriSensitiveColumn::APP_URI_SENSITIVE_TABLE);
...
void CancelUriPermissionTest::TearDown(void)
{
    MEDIA_INFO_LOG("TearDown");
}

static int32_t GrantUriPermission(int32_t fileId)
{
...
static int32_t CancelUriPermission(int32_t fileId)
{
    ...
    MessageParcel reply;
    auto service = make_shared<MediaAssetsControllerService>();
    service->CancelPhotoUriPermission(data, reply);
    ...
    MEDIA_INFO_LOG("CancelUriPermission ErrCode:%{public}d", respVo.GetErrCode());
    return respVo.GetErrCode();
}
...
HWTEST_F(CancelUriPermissionTest, CancelUriPermission_Test_001, TestSize.Level0)
{
    MEDIA_INFO_LOG("CancelUriPermission_Test_001 Begin");
    int32_t result = CancelUriPermission(10000000);
    ASSERT_EQ(result, 0);
}

HWTEST_F(CancelUriPermissionTest, CancelUriPermission_Test_002, TestSize.Level0)
{
    MEDIA_INFO_LOG("CancelUriPermission_Test_002 Begin");
    InsertAsset();
    int32_t fileId = QueryPhotoIdByDisplayName("cam_pic.jpg");
    MEDIA_INFO_LOG("CancelUriPermission_Test_002 fileId = %{public}d", fileId);
    ASSERT_GT(fileId, 0);
    ...
    result = CancelUriPermission(fileId);
    ASSERT_EQ(result, 0);
    result = QueryUriPermissionTableByFileId(fileId);
    ASSERT_LT(result, 0);
}
