...
#include "inner_get_moving_photo_date_modified_test.h"
...
void GetMovingPhotoDateModifiedTest::SetUpTestCase(void)
{
...
void GetMovingPhotoDateModifiedTest::TearDownTestCase(void)
{
    ClearTable(PhotoColumn::PHOTOS_TABLE);
...
void GetMovingPhotoDateModifiedTest::SetUp()
{
    MEDIA_INFO_LOG("SetUp");
}

void GetMovingPhotoDateModifiedTest::TearDown(void)
{
...
static int32_t GetMovingPhotoDateModified(const int32_t& fileId, int64_t &dateModified)
{
    GetMovingPhotoDateModifiedReqBody reqBody;
    reqBody.fileId = to_string(fileId);
    ...
    service->GetMovingPhotoDateModified(data, reply);

    IPC::MediaRespVo<GetMovingPhotoDateModifiedRespBody> resp;
    if (resp.Unmarshalling(reply) != true) {
        MEDIA_ERR_LOG("resp.Unmarshalling failed");
        return -1;
    }

    dateModified = resp.GetBody().dateModified;
    return resp.GetErrCode();
}

HWTEST_F(GetMovingPhotoDateModifiedTest, GetMovingPhotoDateModifiedTest_Test_001, TestSize.Level0)
{
    MEDIA_INFO_LOG("Start GetMovingPhotoDateModifiedTest_Test_001");
    ...
    int32_t ret = GetMovingPhotoDateModified(-1, dateModified);
    EXPECT_EQ(ret, E_ERR);

    ret = GetMovingPhotoDateModified(fileId, dateModified);
    EXPECT_EQ(ret, 0);
    EXPECT_GT(dateModified, 0);
    MEDIA_INFO_LOG("end GetMovingPhotoDateModifiedTest_Test_001");
}

HWTEST_F(GetMovingPhotoDateModifiedTest, GetMovingPhotoDateModifiedTest_Test_002, TestSize.Level0)
{
    MEDIA_INFO_LOG("Start GetMovingPhotoDateModifiedTest_Test_002");
    MessageParcel data;
    MessageParcel reply;
    auto service = make_shared<MediaAssetsControllerService>();
    service->GetMovingPhotoDateModified(data, reply);
    ...
    MEDIA_INFO_LOG("end GetMovingPhotoDateModifiedTest_Test_002");
}
}  // namespace OHOS::Media