...
class GetDirtyTypeStatVoTest : public testing::Test {};

HWTEST_F(GetDirtyTypeStatVoTest, TC001_RespBody_Marshalling_Unmarshalling_Empty, TestSize.Level1)
{
    // 用例说明：测试空数据处理；覆盖边界路径（触发条件：输入为空）；验证业务状态断言：处理成功或失败
    GetDirtyTypeStatRespBody original;
    ...
    parcel.RewindRead(0);
    GetDirtyTypeStatRespBody restored;
...
HWTEST_F(GetDirtyTypeStatVoTest, TC002_RespBody_Marshalling_Unmarshalling_Single, TestSize.Level1)
{
...
HWTEST_F(GetDirtyTypeStatVoTest, TC004_RespBody_Unmarshalling_ReadInt32Size_Fail, TestSize.Level1)
{
    ...
    parcel.RewindRead(0);
    GetDirtyTypeStatRespBody vo;
    bool ret = vo.Unmarshalling(parcel);
    EXPECT_FALSE(ret);
}

HWTEST_F(GetDirtyTypeStatVoTest, TC005_RespBody_Unmarshalling_SizeOverflow, TestSize.Level1)
{
