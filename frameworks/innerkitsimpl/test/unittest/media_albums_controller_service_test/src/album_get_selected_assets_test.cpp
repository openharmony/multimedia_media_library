...
static int32_t ClearTable(const string &table)
{
    ...
    if (err != E_OK) {
        MEDIA_ERR_LOG("zcs Failed to clear album table, err: %{public}d", err);
...
void AlbumGetSelectedAssetsTest::SetUpTestCase(void)
{
    ...
    if (g_rdbStore == nullptr) {
        MEDIA_ERR_LOG("zcs Start MediaLibraryPhotoOperationsTest failed, can not get g_rdbStore");
        exit(1);
    }
...
{
    MEDIA_ERR_LOG("zcs InsertAssetIntoPhotosTable");
    ...
    const string &table, const string &key, const string &value, const vector<string> &columns)
{
    ...
    if (resultSet == nullptr || resultSet->GoToFirstRow() != NativeRdb::E_OK) {
        MEDIA_ERR_LOG("zcs Can not get file asset");
        return nullptr;
    }
    return resultSet;
}
...
static bool executionSql(const std::string &sql){
    ...
    if (ret != NativeRdb::E_OK) {
        MEDIA_ERR_LOG("zcs Execute sql %{public}s failed", sql.c_str());
        return false;
    }
    MEDIA_ERR_LOG("zcs Execute sql %{public}s success", sql.c_str());
    return true;
}
...
{
    // MEDIA_ERR_LOG("zcs InsertAnalysisAlbum");
    // std::string insertSql = "INSERT INTO AnalysisAlbum (album_name, album_subtype, group_tag) VALUES "
    //                         "('"+ albumName + "', 4096, '" + groupTag + "')";
    std::string insertSql = "INSERT INTO AnalysisAlbum (album_name, album_subtype, group_tag) VALUES ('test01', 4096, 'ser_1755057945560169000')";
...
{
    MEDIA_ERR_LOG("zcs InsertAnalysisPhotoMap");
...
{
    MEDIA_ERR_LOG("zcs InsertAnalysisImageFace");
...
static bool CreatTabAnalysisVlm(){
    MEDIA_ERR_LOG("zcs CreatTabAnalysisVlm");
...
static bool InsertTabAnalysisVlm(int fileId){
    MEDIA_ERR_LOG("zcs InsertTabAnalysisVlm");
...
static int32_t QueryAlbumIdByAlbumName(const string &albumName)
{
    ...
    auto resultSet = MediaLibraryRdbStore::Query(rdbPredicates, columns);
    if (resultSet == nullptr || resultSet->GoToFirstRow() != NativeRdb::E_OK) {
        MEDIA_ERR_LOG("zcs Can not get fileId");
        return -1;
    }
...
static bool AlbumGetSelectedAssetsPrepare(int32_t &albumId)
{
    ...
    if (resultSet == nullptr) {
        MEDIA_ERR_LOG("zcs QueryAsset fail");
        return false;
    }
    ...
    MEDIA_ERR_LOG("zcs albumId = %{public}d", albumId);
    ...
    MEDIA_ERR_LOG("zcs albumId = %{public}d", fileId);
...
static int32_t AlbumGetSelectedAssets(int32_t albumId)
{
    ...
    if (reqBody.Marshalling(data) != true) {
        MEDIA_ERR_LOG("zcs reqBody.Marshalling failed");
        return -1;
    }
    ...
    if (resp.Unmarshalling(reply) != true) {
        MEDIA_ERR_LOG("zcs resp.Unmarshalling failed");
        return -1;
    }
    auto err = resp.GetErrCode();
    if (err != E_OK) {
        MEDIA_ERR_LOG("zcs resp.GetErrCode is not E_OK");
        return -1;
    }
    AlbumGetSelectedAssetsRespBody respBody = resp.GetBody();
    if (respBody.resultSet == nullptr) {
        MEDIA_ERR_LOG("zcs respBody.resultSet is nullptr");
        return -1;
    }
    int count = -1;
    ...
    if (errCode != NativeRdb::E_OK) {
        MEDIA_ERR_LOG("zcs ResultSet GetRowCount failed, errCode=%{public}d", errCode);
        return -1;
    }
    return count;
}
