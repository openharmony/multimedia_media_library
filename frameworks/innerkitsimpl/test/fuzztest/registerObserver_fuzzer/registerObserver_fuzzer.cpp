 ...
 */
#include "registerObserver_fuzzer.h"
#include <vector>
#include <fcntl.h>
...
bool RegisterObserverFuzzTest(const uint8_t *data, size_t size)
{
    if ((data == nullptr) || (size <= 0)) {
        return false;
    }

    Uri registerObserverFileUri(std::string(reinterpret_cast<const char*>(data), size));
    ...
    helper->RegisterObserver(registerObserverFileUri, dataObserver);
    return true;
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    OHOS::MediaLibrary::RegisterObserverFuzzTest(data, size);
    return 0;
}