...
static uint16_t PROP_BASE = 0xDC00;
...
static MtpOperationContext FuzzMtpOperationContext(const uint8_t* data, size_t size)
{
    ...
    context.parent = provider->ConsumeIntegral<u_int32_t>();
    context.handle = provider->ConsumeIntegral<u_int32_t>();
...
static void MtpDataUtilsTest(const uint8_t* data, size_t size)
{
    ...
    context->property = provider->ConsumeIntegralInRange<u_int32_t>(MTP_PROPERTY_ALL_CODE - 1, MTP_PROPERTY_ALL_CODE);
    ...
    properties->push_back(PROP_BASE + provider->ConsumeIntegralInRange<uint16_t>(1, 100));
    uint32_t parentId = 0;
    std::unordered_map<uint32_t, std::string> umap{
        {provider->ConsumeIntegral<u_int32_t>(), randomString}
    };
    outProps->clear();
    int32_t storageId = 0;
...
static void InitMtpMedialibraryManager()
{
    shared_ptr<MtpOperationContext> context = make_shared<MtpOperationContext>();
    if (context == nullptr) {
        MEDIA_INFO_LOG("context is nullptr");
        return;
    }
...
static void MtpMedialibraryManagerTest(const uint8_t* data, size_t size)
{
    ...
    uint32_t start = fileId_ > 100 ? fileId_ - 100 : 0;
    ...
    vector<string> ownerAlbumIds = {to_string(provider->ConsumeIntegralInRange<uint32_t>(albumId_, albumId_ + 10))};
    ...
    context->property = provider->ConsumeIntegralInRange<uint16_t>(MTP_PROPERTY_OBJECT_FORMAT_CODE, 
        MTP_PROPERTY_OBJECT_FORMAT_CODE + 255);
    uint64_t outIntVal;
...
static void MtpMedialibraryTest(const uint8_t* data, size_t size)
{
    ...
    context->handle = provider->ConsumeIntegralInRange<u_int32_t>(PTP_IN_MTP_ID, PTP_IN_MTP_ID + 1);
    std::shared_ptr<ObjectInfo> outObjectInfo = std::make_shared<ObjectInfo>(provider->ConsumeIntegral<u_int32_t>());
    std::string randomString = provider->ConsumeBytesAsString(NUM_BYTES);
    mtpMediaLib_->GetGalleryObjectInfo(context, outObjectInfo, randomString);

    context->property = PROP_BASE + provider->ConsumeIntegralInRange<uint16_t>(1, 255);
...
{
    int32_t value = provider->ConsumeIntegralInRange<int32_t>(0, 10);
...
static void DatabaseDataInitial()
{
    ...
    albumValues.PutInt(PhotoAlbumColumns::ALBUM_IS_LOCAL, provider->ConsumeBool() ? 1 : 2);
    ...
    photoValues.PutInt(PhotoColumn::PHOTO_DIRTY, static_cast<int32_t>(FuzzDirtyType()));
