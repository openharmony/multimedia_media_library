...
static void DatabaseDataInitial()
{
    ...
    MEDIA_INFO_LOG("fileId: %{public}ld.", g_fileId);
}
...
static void DatabaseDataClear()
{
    ...
    if (ret != E_OK) {
        MEDIA_ERR_LOG("DeleteAlbumAsset by albumId %{public}ld Failed %{public}d", g_albumId, ret);
    }
    ...
    if (ret != E_OK) {
        MEDIA_ERR_LOG("DeletePhotoAsset by fileId %{public}ld Failed %{public}d", g_fileId, ret);
    }
}
