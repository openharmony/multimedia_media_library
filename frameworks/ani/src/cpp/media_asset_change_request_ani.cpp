...
static bool CheckMovingPhotoCreationArgs(std::unique_ptr<MediaAssetChangeRequestAniContext> &context)
{
    ...
    std::string extension = context->valuesBucket.Get(CONST_ASSET_EXTENTION, isValid);
    if (isValid) {
        return MediaFileUtils::CheckMovingPhotoExtension(extension);
    }

    std::string displayName = context->valuesBucket.Get(CONST_MEDIA_DATA_DB_NAME, isValid);
    ...
    ani_object photoCreateOptions, std::unique_ptr<MediaAssetChangeRequestAniContext> &context)
{
    ...
    CHECK_COND_WITH_RET_MESSAGE(env, mediaType == MEDIA_TYPE_IMAGE || mediaType == MEDIA_TYPE_VIDEO, false,
        "Invalid file type");
    context->valuesBucket.Put(CONST_MEDIA_DATA_DB_NAME, displayNameStr);
    ...
    ani_object createOptions, std::unique_ptr<MediaAssetChangeRequestAniContext> &context)
{
    ...
    context->valuesBucket.Put(CONST_ASSET_EXTENTION, extensionStr);
    context->valuesBucket.Put(CONST_MEDIA_DATA_DB_MEDIA_TYPE, static_cast<int32_t>(mediaType));
...
{
    ANI_INFO_LOG("SetCameraEditData Start");
    if (!MediaLibraryAniUtils::IsSystemApp()) {
        AniError::ThrowError(env, E_CHECK_SYSTEMAPP_FAIL, "This interface can be called only by system apps");
        return nullptr;
    }
...
DataShare::DataShareValuesBucket MediaAssetChangeRequestAni::GetSubmitCacheValuesBucket(
...
int32_t MediaAssetChangeRequestAni::SubmitCache(SubmitCacheConfig submitCacheConfig)
{
    CHECK_COND_RET(fileAsset_ != nullptr, E_FAIL, "Failed to check fileAsset_");
    CHECK_COND_RET(!cacheFileName_.empty() || !cacheMovingPhotoVideoName_.empty(), E_FAIL,
        "Failed to check cache file");

    ANI_INFO_LOG("Check SubmitCache isWriteGpsAdvanced: %{public}d", submitCacheConfig.isWriteGpsAdvanced);

    int32_t ret{E_FAIL};
    SubmitCacheReqBody reqBody;
    reqBody.isWriteGpsAdvanced = submitCacheConfig.isWriteGpsAdvanced;
    SubmitCacheRespBody rspBody;
    ...
    if (submitCacheConfig.isCreation) {
        ...
        reqBody.values = RdbDataShareAdapter::RdbUtils::ToValuesBucket(creationValuesBucket_);
        ret = IPC::UserDefineIPCClient().SetUserId(submitCacheConfig.userId).Call(businessCode, reqBody, rspBody);
    } else {
        DataShare::DataShareValuesBucket valuesBucket = GetSubmitCacheValuesBucket(submitCacheConfig);
    ...
    if (rspBody.fileId > 0 && submitCacheConfig.isCreation) {
...
static bool SubmitCacheExecute(MediaAssetChangeRequestAniContext &context)
{
    ...
    submitCacheConfig.userId = fileAsset->GetUserId();
...
ani_object MediaAssetChangeRequestAni::DeleteLocalAssetsPermanentlyWithUri(ani_env *env,
    [[maybe_unused]] ani_class clazz, ani_object context, ani_object assetUris)
{
    ANI_DEBUG_LOG("enter deleteLocalAssetsPermanentlyWithUri.");
    auto asyncContext = make_unique<MediaAssetChangeRequestAniContext>();
    CHECK_COND_WITH_MESSAGE(env, ParseArgsDeleteLocalAssetsPermanently(
