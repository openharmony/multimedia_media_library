...
#ifndef OHOS_MEDIALIBRARY_MEDIADATA_HELPER_H
#define OHOS_MEDIALIBRARY_MEDIADATA_HELPER_H
...
class MediaDataHelper final : public std::enable_shared_from_this<MediaDataHelper> {
public:
    ~MediaDataHelper() = default;
     ...
     */
    static std::shared_ptr<MediaDataHelper> Creator(const std::shared_ptr<OHOS::AbilityRuntime::Context> &context,
        const AAFwk::Want &want);

    /**
     * @brief Creates a MediaDataHelper instance with the Uri specified based on the given Context.
     *
     * @param context Indicates the Context object on OHOS.
     * @param want Indicates the Want containing information about the target extension ability to connect.
...
private:
    ...
    sptr<IMediaData> mediaDataProxy_ = nullptr;
    bool isSystemCaller_ = false;
    static std::mutex oplock_;
    sptr<IRemoteObject::DeathRecipient> callerDeathRecipient_ = nullptr;
    std::map<sptr<AAFwk::IDataAbilityObserver>, sptr<IMediaData>> registerMap_;
    std::map<sptr<AAFwk::IDataAbilityObserver>, std::string> uriMap_;
    sptr<MediaDataConnection> mediaDataConnection_ = nullptr;
};

class MediaDataDeathRecipient : public IRemoteObject::DeathRecipient {
public:
    using RemoteDiedHandler = std::function<void(const wptr<IRemoteObject> &)>;

    explicit MediaDataDeathRecipient(RemoteDiedHandler handler);

    virtual ~MediaDataDeathRecipient();
...
#endif  // OHOS_MEDIALIBRARY_MEDIADATA_HELPER_H
