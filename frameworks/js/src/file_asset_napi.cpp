...
static unique_ptr<PixelMap> QueryThumbnail(shared_ptr<MediaThumbnailHelper> &thumbnailHelper,
    std::string &uri, Size &size, const std::string &typeMask)
{
    ...
    StartTrace(HITRACE_TAG_FILEMANAGEMENT, "UserFileClient::Query");
...
static void JSGetThumbnailExecute(FileAssetAsyncContext* context)
{
    ...
    CHECK_NULL_PTR_RETURN_VOID(context, "Async context is null");
    if (context->objectInfo->sThumbnailHelper_ != nullptr) {
        ...
        context->pixelmap = QueryThumbnail(context->objectInfo->sThumbnailHelper_, uri, size, context->typeMask);
    } else {
        context->error = ERR_INVALID_OUTPUT;
    ...
    const std::shared_ptr<AbilityRuntime::Context> &context)
{
    ...
    return QueryThumbnail(sThumbnailHelper_, fileUri, size, "");
}
