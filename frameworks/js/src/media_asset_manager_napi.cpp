...
{
    NAPI_DEBUG_LOG("NotifyOnProgress start %{public}d,type %{public}d", progress, type);
    ...
    std::string requestId)
{
    ...
    if (writeData.requestUri.empty() || writeData.destUri.empty()) {
        ...
        NapiError::ThrowError(writeData.env, OHOS_INVALID_PARAM_CODE, "requestUri or responseUri is nullptr");
        return;
    }
    ...
    if (srcFd < 0) {
        ...
        NapiError::ThrowError(writeData.env, OHOS_PERMISSION_DENIED_CODE, "open source file error");
        return;
    }
    struct stat statSrc;
    ...
    if (destFd < 0) {
        ...
        NapiError::ThrowError(writeData.env, OHOS_PERMISSION_DENIED_CODE, "open dest file error");
        return;
    }
    ...
    unique_ptr<MediaAssetManagerAsyncContext> &context, napi_threadsafe_function &progressFunc)
{
    ...
    napi_status status = napi_create_threadsafe_function(env, context->mediaAssetProgressHandler, NULL, workName, 0, 1,
