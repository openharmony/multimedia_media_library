...
static void ParseLivePhotoCoverPosition(std::unique_ptr<Metadata> &data)
{
    ...
    if (MovingPhotoFileUtils::GetVersionAndFrameNum(fd.Get(), version, frameIndex, hasCinemagraphInfo) != E_OK) {
        return;
    }
...
int32_t MetadataExtractor::CombineMovingPhotoMetadata(std::unique_ptr<Metadata> &data)
{
    ...
    uint32_t frameIndex = MovingPhotoFileUtils::GetFrameIndex(videoData->GetCoverPosition(), videoFd.Get());
