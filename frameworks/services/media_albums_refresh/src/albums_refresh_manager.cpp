...
{
    // 媒体库刷新时，读取本次待刷新的用户和来源相册列表，status设置为1
...
// 单批新增数量少于150条，修改数量少于50条，删除数量少于50条
...
// 端云下行资产下行媒体库相册刷新机制, 亮灭屏下行处理和通知策略
    ...
    const std::vector<RefreshAlbumData> &systeAlbums, SyncNotifyInfo &info)
{
    ...
    // 当前屏幕状态下的时间阈值，亮屏10s，灭屏10min
    int32_t delayTime = static_cast<int32_t>(MediaFileUtils::UTCTimeSeconds() - lastRefreshTimestamp_);
    bool notifyAlbums = false, notifyAssets = false;
    for (auto systeAlbum : systeAlbums) {
        PhotoAlbumSubType subtype = static_cast<PhotoAlbumSubType>(systeAlbum.albumSubtype);
        // 单批新增数量少于150条，修改数量少于10条，删除数量少于10条，直接刷新
        MEDIA_DEBUG_LOG("#test notifyType: %{public}d, totalAssets:%{public}d, albumId:%{public}d, size:%{public}zu, "
            ...
            systeAlbums.size(),
            subtype);
        // 调试时需将info.totalAssets改为info.urisSize
        ...
        if (delayTime > timeThreshold) {
            // 超过时间阈值，则处理系统相册（不含图片和视频相册）、用户和来源相册、智慧相册（新增不涉），并只发送相册变更通知
        ...
        // 端云下行资产条目大于阈值，且刷新时间间隔小于阈值，每批数据刷新图片和视频相册，发送一次资产变更和相册变更通知
    ...
    SyncNotifyInfo &info)
{
    ...
    // 通过查询RefreshAlbum中的status为0的字段，获取变化的相册信息
...
{
    // 资产下行新增时，文件系统同步服务没有能力提供fileId，因此给的是cloudId，需要甄别处理下
