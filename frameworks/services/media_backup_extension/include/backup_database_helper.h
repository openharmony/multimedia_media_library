...
public:
    ...
    struct FileQueryInfo {
        ...
        FileQueryInfo(int32_t dbType, const std::string &tableName, const std::string &columnName,
            const std::string &path) : tableName(tableName), columnName(columnName), path(path) {}
    };
...
public:
    ...
    void IsFileExist(int32_t sceneCode, const FileInfo &fileInfo, int32_t &dbStatus, int32_t &fileStatus);
...
private:
    ...
    const std::unordered_map<int32_t, DbInfo> DB_INFO_MAP = {
        { DbType::PHOTO_CACHE, DbInfo("photo_Cache", "/storage/emulated/0/photo_Cache.db") },
        { DbType::VIDEO_CACHE, DbInfo("video_Cache", "/storage/emulated/0/video_Cache.db") },
        { DbType::PHOTO_SD_CACHE, DbInfo("photo_sd_Cache", "/storage/emulated/0/photo_sd_Cache.db") },
        { DbType::VIDEO_SD_CACHE, DbInfo("video_sd_Cache", "/storage/emulated/0/video_sd_Cache.db") },
    };
};
} // namespace OHOS::Media
