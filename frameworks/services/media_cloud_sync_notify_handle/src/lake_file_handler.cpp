...
static int32_t QueryLakeData(const vector<string> &fileIdList, vector<LakeData> &lakeDataList)
{
    ...
    predicates.IsNotNull(PhotoColumn::PHOTO_STORAGE_PATH)->And()->In(PhotoColumn::MEDIA_ID, fileIdList);
...
static void HandleStoragePath(const LakeData &data)
{
    string lpath;
    int errCode = GetLpath(data.storagePath, lpath);
    if (errCode != E_OK) {
    ...
    size_t lastSlash = data.storagePath.rfind('/');
    if (lastSlash == string::npos) {
        MEDIA_ERR_LOG("slash not found in storage path: %{public}s", data.storagePath.c_str());
        return;
    }
    string displayName = data.storagePath.substr(lastSlash + 1);
    if (data.lpath == lpath && data.displayName == displayName) {
        return;
    }

    string storagePath = "/storage/media/local/files/Docs/HO_DATA_EXT_MISC" + data.lpath + "/" + data.displayName;
    MEDIA_INFO_LOG("new storagePah is %{public}s, old lpath: %{public}s, old name: %{public}s",
        storagePath.c_str(), lpath.c_str(), displayName.c_str());
    RdbPredicates predicates(PhotoColumn::PHOTOS_TABLE);
    predicates.EqualTo(PhotoColumn::MEDIA_ID, data.fileId);
    ValuesBucket values;
    values.PutString(PhotoColumn::PHOTO_STORAGE_PATH, storagePath);
    int32_t changedRows = -1;
    auto rdbStore = MediaLibraryUnistoreManager::GetInstance().GetRdbStore();
    CHECK_AND_RETURN_LOG(rdbStore != nullptr, "update storagePath error");
    int32_t ret = rdbStore->Update(changedRows, values, predicates);
    CHECK_AND_RETURN_LOG(ret == E_OK && changedRows > 0,
        "update path error, ret: %{public}d, change rows: %{public}d",
        ret, changedRows);
    return;
}

static void HandleMetaChanged(const vector<string> &metaChangedFileIds)
{
    if (metaChangedFileIds.empty()) {
        return;
    }

    vector<LakeData> lakeDataList;
    int32_t errCode = QueryLakeData(metaChangedFileIds, lakeDataList);
    if (errCode != E_OK) {
        return;
    }
    ...
    for (auto &lakeData : lakeDataList) {
        if (!IsValidLakeStoragePath(lakeData.storagePath)) {
            MEDIA_WARN_LOG("Failed to check storagePath of fileId: %{public}d", lakeData.fileId);
            continue;
        }
...
static void HandleAssetContentChanged(const vector<string> &assetContentChangedFileIds)
{
    if (assetContentChangedFileIds.empty()) {
        return;
    }

    vector<LakeData> lakeDataList;
    int32_t errCode = QueryLakeData(assetContentChangedFileIds, lakeDataList);
    if (errCode != E_OK) {
        return;
    }

    MEDIA_INFO_LOG("%{public}d lake data changes asset content", static_cast<int32_t>(lakeDataList.size()));
    for (auto &lakeData : lakeDataList) {
        if (!IsValidLakeStoragePath(lakeData.storagePath)) {
            MEDIA_WARN_LOG("Failed to check storagePath of fileId: %{public}d", lakeData.fileId);
            continue;
        }
        HandleStoragePath(lakeData);
    }
}

void LakeFileHandler::Handle(const CloudSyncHandleData &handleData)
{
    vector<string> metaChangedFileIds;
    vector<string> assetContentChangedFileIds;
    for (auto &notifyInfo : handleData.notifyInfo) {
        MEDIA_DEBUG_LOG("cloud_lake debug: notify type is %{public}d", static_cast<int32_t>(notifyInfo.first));
        if (notifyInfo.first != static_cast<ChangeType>(NotifyType::NOTIFY_UPDATE)) {
            return;
        }

        for (auto &uriRemarkPair : notifyInfo.second) {
            string remark = uriRemarkPair.second;
            string id = MediaFileUtils::GetIdFromUri(uriRemarkPair.first.ToString());
            if (remark == "meta") {
                MEDIA_DEBUG_LOG("cloud_lake debug: meta changed of file %{public}s", id.c_str());
                metaChangedFileIds.push_back(id);
            }

            if (remark == "asset") {
                MEDIA_DEBUG_LOG("cloud_lake debug: asset content changed of file %{public}s", id.c_str());
                assetContentChangedFileIds.push_back(id);
            }
        }
    }

    HandleMetaChanged(metaChangedFileIds);
    HandleAssetContentChanged(assetContentChangedFileIds);
