...
{
    CHECK_AND_RETURN_RET_INFO_LOG(!data.rdbUpdateCache.IsEmpty(), E_OK, "RdbUpdateCache is empty, no need update: id: %{public}s,
        path: %{public}s", data.id.c_str(), DfxUtils::GetSafePath(data.path).c_str());
