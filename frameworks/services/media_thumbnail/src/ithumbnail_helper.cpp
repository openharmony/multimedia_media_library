...
bool IThumbnailHelper::TrySavePixelMap(ThumbnailData &data, ThumbnailType type)
{
    if (!data.needCheckWaitStatus) {
        ...
        CHECK_AND_RETURN_RET_LOG(
            err == E_OK, false, "No wait TrySavePixelMap failed: %{public}d, path: %{public}s", err,
            DfxUtils::GetSafePath(data.path).c_str());
        return true;
    }
...
bool IThumbnailHelper::SaveLcdPictureSource(ThumbRdbOpt &opts, ThumbnailData &data, bool isSourceEx)
{
    ...
    CHECK_AND_RETURN_RET_LOG(ThumbnailUtils::CompressPicture(data, isSourceEx, tempOutputPath), false,
        "SaveLcdPictureSource failed, CompressPicture failed");
    CHECK_AND_RETURN_RET_LOG(TrySavePicture(data, isSourceEx, tempOutputPath), false,
        "SaveLcdPictureSource failed, save picture failed");
...
bool IThumbnailHelper::CacheFailState(const ThumbRdbOpt &opts, ThumbnailData &data)
{
    ...
    data.rdbUpdateCache.PutLong(
        PhotoColumn::PHOTO_THUMBNAIL_READY, static_cast<int64_t>(ThumbnailReady::GENERATE_THUMB_RETRY));
...
int32_t IThumbnailHelper::CacheDirtyState(const ThumbRdbOpt &opts, ThumbnailData &data)
{
    ...
    data.rdbUpdateCache.PutInt(
        PhotoColumn::PHOTO_DIRTY, shouldUpdateFDirty ? static_cast<int32_t>(DirtyType::TYPE_FDIRTY) :
