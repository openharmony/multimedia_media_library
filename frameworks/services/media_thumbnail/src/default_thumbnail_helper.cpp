...
int32_t DefaultThumbnailHelper::CreateThumbnail(ThumbRdbOpt &opts, bool isSync)
{
    ...
    string fileName = ThumbnailUtils::GetThumbPath(thumbnailData.path, THUMBNAIL_THUMB_SUFFIX);
...
int32_t DefaultThumbnailHelper::GetThumbnailPixelMap(ThumbRdbOpt &opts)
{
    ...
    if (access(fileName.c_str(), F_OK) != 0) {
        ...
        if (!opts.path.empty()) {
            fileName = ThumbnailUtils::GetThumbPath(thumbnailData.path, THUMBNAIL_THUMB_SUFFIX);
        }
    }
