...
int32_t MediaScannerAbilityProxy::ScanDirService(std::string &scanDirPath, const sptr<IRemoteObject> &scanDirCb)
{
    ...
    if (error != ERR_NONE) {
        MEDIA_ERR_LOG("%{public}s:: ScanDirService IPC failed, error: %{public}d", __func__, error);
        return SCAN_IPC_ERR;
    }
...
int32_t MediaScannerAbilityProxy::ScanFileService(std::string &scanFilePath, const sptr<IRemoteObject> &scanFileCb)
{
    ...
    if (error != ERR_NONE) {
        MEDIA_ERR_LOG("%{public}s:: ScanFileService IPC failed, error: %{public}d", __func__, error);
        return SCAN_IPC_ERR;
    }
