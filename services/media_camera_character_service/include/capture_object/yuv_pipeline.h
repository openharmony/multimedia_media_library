/*
 * Copyright (c) 2026 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef OHOS_MEDIA_YUV_PIPELINE_H
#define OHOS_MEDIA_YUV_PIPELINE_H

#include <stdint.h>
#include <string>

#include "camera_asset_pipeline.h"

namespace OHOS::Media {
struct TempModifiedDataForYuv {
    bool isModified = false;
    std::string modifyMimeType;
    std::string modifyFilePath;
    std::string modifyDisplayName;
    std::string modifyExtension;

    void Clear()
    {
        isModified = false;
        modifyMimeType.clear();
        modifyFilePath.clear();
        modifyDisplayName.clear();
        modifyExtension.clear();
    }
};

class YuvPipeline : public CameraAssetPipeline {
public:
    EXPORT YuvPipeline();
    virtual ~YuvPipeline() = default;

private:
    void AddFiltersToPhoto(const std::string &inputPath, const std::string &outputPath,
        const std::string &editdata, const std::string &photoStatus = "");

    // 一阶段上报
    virtual bool CloseCameraFileFdWithMutex(const std::string& realPath, const std::string& tempPath,
        const CameraPathType& pathType) override;
    virtual void OnDelivery(std::shared_ptr<Media::Picture> picture) override;

    // 一阶段落盘
    virtual bool UpdateExtValuesForStageInternal(const SaveCameraPhotoDto &dto, NativeRdb::ValuesBucket &values,
        CameraAssetInfo& modifyAssetInfo) override;
    virtual void SaveImageForStageInternal(const SaveCameraPhotoDto& dto) override;
    virtual void ScanFileForStageInternal() override;
    virtual int32_t CheckSaveImageForYuv() override;

    int32_t SavePictureForFirstStage(bool needDfx);
    int32_t SaveOnePictureForFirstStage(const std::string& filePath, const std::string& mimeType, bool needDfx);
    int32_t SaveTwoPictureForFirstStage(const std::string& filePath, const std::string& mimeType,
        std::string& editData, bool needDfx);

    // 二阶段落盘
    virtual bool InitForOnProcessInternal(const OnProcessImageWrapper &wrapper) override;
    virtual bool CheckCanSaveDirectlyInternal(const std::shared_ptr<FileAsset> &fileAsset) override;
    virtual int32_t ProcessMultistagesPhotoInternal(const std::shared_ptr<FileAsset> &fileAsset) override;
    virtual void ScanFileForOnProcessInternal() override;

    int32_t SaveOnePictureForOnProcess(const CameraAssetInfo& assetInfo, std::shared_ptr<Media::Picture> &picture);
    int32_t SaveTwoPictureForOnProcess(const CameraAssetInfo& assetInfo, std::shared_ptr<Media::Picture> &picture);

    std::mutex fileMutex_;

    // 一阶段临时数据(一阶段结束时, 需要清理)
    TempModifiedDataForYuv tempData_;
    std::shared_ptr<Media::Picture> resultPictureForFirstStage_;
    bool isHighForEffective_{false};
    bool isHighForSource_{false};

    // 二阶段数据
    OnProcessParamForYuv yuv_;
    std::shared_ptr<Media::Picture> resultPictureForOnProcess_;
    bool isSaveDirectly_{true};
};
} // namespace OHOS::Media
#endif // OHOS_MEDIA_YUV_PIPELINE_H
