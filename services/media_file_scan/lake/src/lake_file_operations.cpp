...
constexpr int32_t ERRNO_FILE_NOT_EXIST = 2;
constexpr int64_t LAKE_MODIFY_TIME_THRESHOLD_MS = 5000;
// LCOV_EXCL_START
...
int32_t LakeFileOperations::MoveLakeFile(const std::string &srcPath, const std::string &destPath)
{
    ...
    size_t destPos = destPath.find_last_of('/');
    CHECK_AND_RETURN_RET_LOG(destPos != std::string::npos, 0, "destPath not contain '/'");
    std::string parentDir = destPath.substr(0, destPos);
    ...
    std::string &newTitle, std::string &newDisplayName)
{
    ...
    if (leftParentheses != std::string::npos && rightParentheses != std::string::npos &&
        ...
        std::string numStr = fileName.substr(leftParentheses + 1, rightParentheses - leftParentheses - 1);
...
static int32_t CreateParentDir(int32_t &targetAlbumId, std::string &parentDir)
{
    ...
    auto resultSet = MediaLibraryRdbStore::Query(predicates,
        { PhotoAlbumColumns::ALBUM_LPATH, PhotoAlbumColumns::ALBUM_SUBTYPE });
    CHECK_AND_RETURN_RET_LOG(resultSet != nullptr, E_ERR, "query database error");
...
const std::string DOCS_DIR = "/storage/emulated/0/FromDocs/";
...
static bool CheckSubDirForFileManager(const std::string &path, size_t startPos)
{
    ...
    CHECK_AND_RETURN_RET(!subDirName.empty(), false);
    return FILE_MANAGER_EXCLUDED_DIR_NAMES.count(subDirName) == 0;
}

static bool CheckFileManagerRealPath(const std::string &path)
{
    ...
    shared_ptr<NativeRdb::ResultSet> &resultSet, std::vector<MoveAssetsToLakeUpdateData> &updateDatas)
{
    ...
    if (outerLakePath.empty() || innerLakePath.empty()) {
     ...
     //排查文管目录
    if (CheckFileManagerRealPath(sourcePath)) {
        MEDIA_INFO_LOG("file is file manager, path: %{public}s", DfxUtils::GetSafePath(sourcePath).c_str());
        return E_OK;
    }
    ...
    AccurateRefreshBase &refresh, const std::vector<std::string> &ids, int32_t targetAlbumId)
{
    ...
    for (MoveAssetsToLakeUpdateData innerLakeAsset : innerLakeAssets) {
        ...
        MEDIA_INFO_LOG("%{public}s, %{public}s", FileScanUtils::GarbleFilePath(originInnerLakePath).c_str(),
            FileScanUtils::GarbleFilePath(innerLakeAsset.storagePath).c_str());
        if (ret < 0) {
...
int32_t LakeFileOperations::MoveAssetsToLake(AccurateRefreshBase &refresh, const std::vector<std::string> &ids)
{
    ...
    if (rowCount == 0) {
        MEDIA_DEBUG_LOG("move asset row count is 0");
        resultSet->Close();
        return E_OK;
    }
...
static int32_t GetEditPathAndEditTime(const std::string& fileUri, std::string& editPath, int64_t& editTime)
{
    ...
    std::string  logicPathPrefixInLake = FileScanUtils::GetCurrentInLakeLogicPrefix();
    size_t logicPathPrefixLen = logicPathPrefixInLake.size();

    std::string storagePath;
    std::string pathPreFix(IN_LAKE_MOUNT_OUTLAKE_PATH_PREFIEX);
    if (fileUri.size() > logicPathPrefixLen &&
        fileUri.compare(0, logicPathPrefixLen, logicPathPrefixInLake) == 0) {
        storagePath = pathPreFix + fileUri.substr(logicPathPrefixLen);
    }
    ...
    const std::string &displayName, const std::string &storagePath, const std::string &data)
{
    ...
    int32_t ret = FileScanUtils::RenameFileCrossPolicy(storagePath, newPath);
    ...
    } else if (errno == ERRNO_FILE_NOT_EXIST) { // storagePath not exist: trashed, hidden, cloud
