...
class FolderParser {
public:
    FolderParser(const std::string &storagePath, ScanMode scanType = ScanMode::INCREMENT);
    virtual ~FolderParser() = default;
    
    FolderOperationType PreProcessFolder();
    int32_t InsertAlbumInfo();
    int32_t UpdatePhotoAlbum();
    CommonAlbumInfo GetAlbumInfo();
...
private:
    int32_t GetAlbumPhotoInfo(CommonAlbumInfo &photoAlbumInfo);
    ...
    int32_t InsertAlbum(CommonAlbumInfo &commonAlbumInfo);
    int32_t UpdateAlbum(const CommonAlbumInfo &commonAlbumInfo, const NativeRdb::AbsRdbPredicates &predicates);
    int32_t DeleteAlbum(const NativeRdb::AbsRdbPredicates &predicates);
    long long GetTimestampMs();

    virtual int32_t GetConvertedLpath(const std::string &data, std::string &lpath) = 0;
    virtual bool IsFolderSkip() = 0;
