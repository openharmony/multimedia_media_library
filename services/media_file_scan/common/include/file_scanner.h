/*
 * Copyright (C) 2025 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
 

#ifndef FILE_SCANNER_H
#define FILE_SCANNER_H

#include <vector>
#include <string>

#include "file_parser.h"
#include "folder_parser.h"
#include "values_bucket.h"
#include "media_file_notify_info.h"

namespace OHOS::Media {

const int32_t RET_SUCCESS = 0;

struct FolderParserInfo {
    shared_ptr<FolderParser> folderParser_;
    std::string currentFolderPath_;
    FolderOperationType currentOptType_{FolderOperationType::SKIP};
    bool isSkipFolder = false;
};

class FileScanner {
public:
    FileScanner(ScanMode scanMode = ScanMode::INCREMENT);
    virtual ~FileScanner() = default;
    int32_t Run(std::vector<MediaNotifyInfo> fileInfos);
    int32_t GetAddCount() const;
    int32_t GetUpdateCount() const;
    int32_t GetDeleteCount() const;
    int32_t GetAlbumAddCount() const;
    int32_t GetAlbumUpdateCount() const;

    static void DeleteRelatedResource(InnerFileInfo &fileIinfo);

protected:
    void GetInsertAssetInfo(MediaNotifyInfo &fileInfo, FileParser &fileParser);
    void RefreshAssetInfoForSkipFile(FileParser &fileParser, FileUpdateType type);
    void RefreshUpdateAssetAlbumInfo(MediaNotifyInfo &fileInfo, FileParser &fileParser);
    void RefreshUpdateAssetInfo(FileParser &fileParser);

private:
    virtual void HandleFiles(MediaNotifyInfo& fileInfo) = 0;
    virtual bool IsIncrementScanConflict(std::vector<MediaNotifyInfo> fileInfos) = 0;
    virtual bool IsSkipDirectory(const std::string &dir) = 0;
    // 工厂方法实例创建
    virtual std::shared_ptr<FolderParser> BuildFolderParser(const std::string &path) = 0;

    std::string GetFolder(std::string file);
    virtual void RefreshInsertAssetInfo();
    void UpdateAlbum();
    bool CheckUpdateFolderParserInfo(MediaNotifyInfo &fileInfo);

protected:
    const ScanMode scanMode_;
    int32_t addCount_{0};
    int32_t updateCount_{0};
    int32_t deleteCount_{0};
    int32_t albumAddCount_{0};
    int32_t albumUpdateCount_{0};
    vector<std::string> inodes_;
    vector<NativeRdb::ValuesBucket> insertFileInfos_;

private:
    std::vector<std::string> albumIds_;
    FolderParserInfo folderParserInfo_;
};
}

#endif