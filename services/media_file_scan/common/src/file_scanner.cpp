...
int32_t FileScanner::Run(vector<MediaNotifyInfo> fileInfos)
{
...
void FileScanner::RefreshUpdateAssetInfo(FileParser &fileParser)
{
    ...
    FileScanUtils::UpdateAndNotifyAnalysisAlbum({to_string(fileInfo.fileId)});
...
void FileScanner::RefreshUpdateAssetAlbumInfo(MediaNotifyInfo &notifyFileInfo, FileParser &fileParser)
{
    ...
    CommonAlbumInfo albumInfo = folderParser->GetAlbumInfo();
    auto fileInfo = fileParser.GetFileInfo();
...
bool FileScanner::CheckUpdateFolderParserInfo(MediaNotifyInfo &fileInfo)
{
    ...
    folderParserInfo_.folderParser_ = BuildFolderParser(folderPath);
...
int32_t FileScanner::GetAddCount() const
{
    return addCount_;
}

int32_t FileScanner::GetUpdateCount() const
{
    return updateCount_;
}

int32_t FileScanner::GetDeleteCount() const
{
    return deleteCount_;
}

int32_t FileScanner::GetAlbumAddCount() const
{
    return albumAddCount_;
}

int32_t FileScanner::GetAlbumUpdateCount() const
{
