...
bool FolderScannerUtils::IsSkipCurrentFile(const std::string &filePath, const ScanRuleConfig &ruleConfig)
{
...
bool FolderScannerUtils::ShouldScanDirectory(const std::string &filePath, const ScanRuleConfig &ruleConfig)
{
    ...
    //CASE 1
    if (std::regex_match(filePath, ruleConfig.visiblePattern)) {
    ...
    if (ruleConfig.createNomediaForInvisibleDirectory && std::regex_match(filePath, ruleConfig.invisiblePattern)) {
        ...
        std::string tmpPath;
        CHECK_AND_RETURN_RET_LOG(PathToRealPath(nomediaPath, tmpPath), false,
...
std::string ExtractRelativePath(const std::string &path, const ScanRuleConfig &ruleConfig)
{
...
void FolderScannerUtils::CleanNomediaInDefaultDirs(const std::string &dirPath, const ScanRuleConfig &ruleConfig)
{
    if (!ruleConfig.cleanNomediaInDefaultDirs) {
        return;
    }
    ...
    bool isTargetFirstLevelChild = (segments.size() == 2 && ruleConfig.defaultFolderNames.count(segments[0])
...
bool FolderScannerUtils::IsSkipCurrentDirectory(const std::string &currentDir, const ScanRuleConfig &ruleConfig)
{
    ...
    CleanNomediaInDefaultDirs(currentDir, ruleConfig);
...
bool FolderScannerUtils::IsSkipDirectory(const std::string &dir, const ScanRuleConfig &ruleConfig)
{
    ...
    return IsSkipCurrentDirectory(currentDir, ruleConfig);
}

bool FolderScannerUtils::IsCurrentOrParentDir(const std::string &dir)
{
    return dir == "." || dir == "..";
}
} // namespace OHOS::Media