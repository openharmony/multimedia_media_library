...
int MoveObjectData::Parser(const std::vector<uint8_t> &buffer, int32_t readSize)
{
    ...
    CHECK_AND_RETURN_RET_LOG(MtpPacketTool::GetUInt32(buffer, offset, context_->handle),
        MTP_ERROR_PACKET_INCORRECT, "MoveObjectData::parser get handle failed");
    CHECK_AND_RETURN_RET_LOG(MtpPacketTool::GetUInt32(buffer, offset, context_->storageID),
        MTP_ERROR_PACKET_INCORRECT, "MoveObjectData::parser get storageID failed");
    CHECK_AND_RETURN_RET_LOG(MtpPacketTool::GetUInt32(buffer, offset, context_->parent),
        MTP_ERROR_PACKET_INCORRECT, "MoveObjectData::parser get parent failed");
    return MTP_SUCCESS;
}
