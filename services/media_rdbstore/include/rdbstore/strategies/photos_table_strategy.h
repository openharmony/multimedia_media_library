...
#ifndef OHOS_PHOTOS_TABLE_STRATEGY_H
#define OHOS_PHOTOS_TABLE_STRATEGY_H
...
namespace OHOS {
namespace Media {
class PhotosTableStrategy final : public RdbTableStrategy {
public:
    PhotosTableStrategy() = default;
    ~PhotosTableStrategy() override = default;
...
protected:
    ...
    virtual TableStrategyErrno ExtendDeleteValues(NativeRdb::ValuesBucket& values,
        const TableStrategyConfig &config) override;
    virtual int32_t ExtendUpdateValues(NativeRdb::ValuesBucket& values, const TableStrategyConfig &config) override;
    virtual std::string GetQueryFilter(const TableStrategyConfig &config) const override;

private:
    void AddDefaultInsertPhotoValues(NativeRdb::ValuesBucket& values);
};
} // namespace Media
} // namespace OHOS

#endif // OHOS_PHOTOS_TABLE_STRATEGY_H