...
bool GetFilePathFromUriReqBody::Unmarshalling(MessageParcel &parcel)
{
    bool status = parcel.ReadString(this->virtualId);
    CHECK_AND_RETURN_RET(status, status);
    return true;
}

bool GetFilePathFromUriReqBody::Marshalling(MessageParcel &parcel) const
{
    bool status = parcel.WriteString(this->virtualId);
    CHECK_AND_RETURN_RET(status, status);
    return true;
}
...
bool GetFilePathFromUriRespBody::Unmarshalling(MessageParcel &parcel)
{
    ...
    MEDIA_INFO_LOG("ReadFromParcel success");
    return true;
}
