...
bool ChangeRequestMoveAssetsByPathReqBody::Unmarshalling(MessageParcel &parcel)
{
    if (!IPC::ITypeMediaUtil::UnmarshalStrVec(assetPaths, parcel)) {
        MEDIA_ERR_LOG("Failed to unmarshall assets");
        return false;
    }
    if (!parcel.ReadString(targetAlbumId)) {
...
bool ChangeRequestMoveAssetsByPathReqBody::Marshalling(MessageParcel &parcel) const
{
    if (!IPC::ITypeMediaUtil::MarshalStrVec(assetPaths, parcel)) {
        MEDIA_ERR_LOG("Failed to marshall assets");
        return false;
    }
    if (!parcel.WriteString(targetAlbumId)) {
...
bool ChangeRequestMoveAssetsByPathRespBody::Unmarshalling(MessageParcel &parcel)
{
...
bool ChangeRequestMoveAssetsByPathRespBody::Marshalling(MessageParcel &parcel) const
{
