...
public:
    ...
    EXPORT void StartProcessMsgs();
    EXPORT void StopProcessMsgs();

protected:
    MediaFileChangeProcessor();
    void ProcessFileChanged();
    EXPORT void ProcessSingleFileChange(const FileMonitorService::FileMsgModel &fileInfo);
    ...
    struct PathPrefixMapping {
        std::string logicPrefix;
        std::string physicalPrefix;
    };
    std::optional<PathPrefixMapping> GetPathPrefixMapping(const std::string &uri) const;
    bool IsInDocsPath(const std::string &uri) const;
    std::string BuildStoragePath(const std::string &uri) const;
    MediaNotifyInfo BuildLakeNotifyInfo(const FileMonitorService::FileMsgModel &fileInfo);
