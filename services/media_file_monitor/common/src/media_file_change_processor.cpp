...
#define FILE_MANAGER_DOCS_PATH_PREFIEX "/storage/media/local/files/Docs/"
...
void MediaFileChangeProcessor::ProcessFileChanged()
{
    ...
    while (true) {
        ...
        CHECK_AND_RETURN_INFO_LOG(!msgs.empty(), "No file change messages found.");
...
{
    if (!IsInDocsPath(fileInfo.fileUri)) {
...
#endif
    CHECK_AND_RETURN_LOG(fileInfo.isContentChange || IsInDocsPath(fileInfo.oldFileUri),
...
std::optional<MediaFileChangeProcessor::PathPrefixMapping> MediaFileChangeProcessor::GetPathPrefixMapping(
    const std::string &uri) const
{
    // Lake 路径映射
    std::string lakeLogicPrefix = FileScanUtils::GetCurrentInLakeLogicPrefix();
    if (uri.compare(0, lakeLogicPrefix.size(), lakeLogicPrefix) == 0) {
        return PathPrefixMapping {
            .logicPrefix = lakeLogicPrefix,
            .physicalPrefix = IN_LAKE_MOUNT_OUTLAKE_PATH_PREFIEX
        };
    }

    // FileManager 路径映射
    std::string fileManagerLogicPrefix = "/data/service/el2/" +
        std::to_string(FileScanUtils::GetCurrentAccountId()) + "/hmdfs/account/files/Docs/";
    if (uri.compare(0, fileManagerLogicPrefix.size(), fileManagerLogicPrefix) == 0) {
        return PathPrefixMapping {
            .logicPrefix = fileManagerLogicPrefix,
            .physicalPrefix = FILE_MANAGER_DOCS_PATH_PREFIEX
        };
    }
...
bool MediaFileChangeProcessor::IsInDocsPath(const std::string &uri) const
{
    return GetPathPrefixMapping(uri).has_value();
}

std::string MediaFileChangeProcessor::BuildStoragePath(const std::string &uri) const
{
    auto mapping = GetPathPrefixMapping(uri);
    CHECK_AND_RETURN_RET_LOG(mapping.has_value(), "",
        "BuildStoragePath invalid uri: %{public}s", DfxUtils::GetSafePath(uri).c_str());

    const auto &logicPrefix = mapping->logicPrefix;
    CHECK_AND_RETURN_RET_LOG(uri.size() > logicPrefix.size(), "",
        "BuildStoragePath invalid uri length: %{public}s", DfxUtils::GetSafePath(uri).c_str());

    return mapping->physicalPrefix + uri.substr(logicPrefix.size());
}

MediaNotifyInfo MediaFileChangeProcessor::BuildLakeNotifyInfo(const FileMonitorService::FileMsgModel &fileInfo)
{
    auto objType = fileInfo.isFile ? FileNotifyObjectType::FILE : FileNotifyObjectType::DIRECTORY;
    MediaNotifyInfo info {
        .beforePath = BuildStoragePath(fileInfo.oldFileUri),
        .afterPath  = BuildStoragePath(fileInfo.fileUri),
        .objType    = objType,
