...
using ProcessorKey = std::tuple<FileNotifyObjectType, FileNotifyOperationType, FileNotifyPathType>;
...
struct ProcessorKeyHash {
    std::size_t operator()(const ProcessorKey& key) const
    {
        ...
        auto h2 = std::hash<int>{}(static_cast<int>(std::get<1>(key)));
...
public:
    ...
    {
        std::lock_guard<std::mutex> lock(mutex_);
        bool needPostProcess = (std::get<1>(key) == FileNotifyOperationType::MOD);
