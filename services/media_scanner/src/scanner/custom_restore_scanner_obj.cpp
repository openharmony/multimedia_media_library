...
#define MLOG_TAG "CustomRestoreScannerObj"
...
CustomRestoreScannerObj::CustomRestoreScannerObj(CustomRestoreInfo& info)
    : customRestoreInfo_(info)
{
}

int32_t CustomRestoreScannerObj::Execute()
{
    MEDIA_INFO_LOG("CustomRestoreScannerObj::Execute begin, file count: %{public}d",
...
int32_t CustomRestoreScannerObj::ResolveMetadata()
{
    ...
    for (const auto &fileInfo : fileInfos) {
        CustomRestoreItem item;
        item.fileInfo = fileInfo;
        item.metadata = std::make_unique<Metadata>();

        int32_t err = CustomRestoreUtils::FillMetadata(timeInfoMap, fileInfo, item.metadata);
        if (err != E_OK) {
...
int32_t CustomRestoreScannerObj::Deduplicate()
{
    int32_t dupCount = 0;

    // Back-fill fileInfo from metadata and collect for batch dedup
    std::vector<RestoreFileInfo> fileInfos;
    fileInfos.reserve(items_.size());
    for (auto &item : items_) {
        ...
        fileInfos.push_back(item.fileInfo);
    }
    auto dupKeys = CustomRestoreUtils::BatchQueryDuplicates(customRestoreInfo_, fileInfos);

    // Collect duplicate items for batch UPDATE
    std::vector<std::pair<std::string, std::string>> dupUpdatePairs; // {filePath, uuid}
    for (auto &item : items_) {
        std::string key = item.fileInfo.fileName + "_" + std::to_string(item.fileInfo.size) + "_" +
                          std::to_string(item.fileInfo.mediaType) + "_" + std::to_string(item.fileInfo.orientation);
        if (dupKeys.count(key) > 0) {
            item.isDuplicate = true;
            dupCount++;
            dupUpdatePairs.emplace_back(item.fileInfo.filePath, MediaFileUtils::GenerateUUID());
        }
    }
...
int32_t CustomRestoreScannerObj::ConvertToValues()
{
    for (auto &item : items_) {
        if (item.isDuplicate) {
            continue;
        ...
        CustomRestoreUtils::SetTimeInfo(item.metadata, item.values);
...
// Step 4: Batch insert to database
int32_t CustomRestoreScannerObj::Insert()
{
...
void CustomRestoreScannerObj::PostProcess()
{
