...
private:
    const std::string SQL_PHOTO_ALBUM_COUNT_FOR_CLONE_LOCAL = "\
        ...
        WHERE PhotoAlbum.dirty <> ? AND \
            (P1.file_id IS NOT NULL AND P1.position IN (1, 3) AND P1.file_source_type IN (0, 1, 3) OR \
            P2.file_id IS NOT NULL AND P2.position IN (1, 3) AND P2.file_source_type IN (0, 1, 3)) ;";
    ...
    const std::string SQL_PHOTO_ALBUM_COUNT_FOR_CLONE_LOCAL_AND_CLOUD = "\
        ...
        FROM PhotoAlbum \
            ...
            ON PhotoAlbum.album_id=P2.owner_album_id \
        WHERE PhotoAlbum.dirty <> ? AND \
            (P1.file_id IS NOT NULL AND P1.position IN (1, 2, 3) AND P1.file_source_type IN (0, 1, 3) OR \
            P2.file_id IS NOT NULL AND P2.position IN (1, 2, 3) AND P2.file_source_type IN (0, 1, 3)) ;";
