...
bool ChangeRequestDismissAssetsReqBody::Unmarshalling(MessageParcel &parcel)
{
    bool status = IPC::ITypeMediaUtil::UnmarshalStrVec(this->assets, parcel);
    CHECK_AND_RETURN_RET(status, status);
    parcel.ReadInt32(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    parcel.ReadInt32(this->photoAlbumSubType);
    CHECK_AND_RETURN_RET(status, status);

    return true;
}

bool ChangeRequestDismissAssetsReqBody::Marshalling(MessageParcel &parcel) const
{
    bool status = IPC::ITypeMediaUtil::MarshalStrVec(this->assets, parcel);
    CHECK_AND_RETURN_RET(status, status);
    parcel.WriteInt32(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    parcel.WriteInt32(this->photoAlbumSubType);
    CHECK_AND_RETURN_RET(status, status);
