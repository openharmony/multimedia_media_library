...
#define MLOG_TAG "MediaSetSubtitleVo"
...
using namespace std;
bool SetSubtitleReqBody::Unmarshalling(MessageParcel &parcel)
{
    bool status = parcel.ReadString(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    status = parcel.ReadString(this->subtitle);
    CHECK_AND_RETURN_RET(status, status);
...
bool SetSubtitleReqBody::Marshalling(MessageParcel &parcel) const
{
    bool status = parcel.WriteString(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    status = parcel.WriteString(this->subtitle);
    CHECK_AND_RETURN_RET(status, status);
