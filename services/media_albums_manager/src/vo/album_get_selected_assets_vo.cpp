...
bool AlbumGetSelectedAssetsReqBody::Unmarshalling(MessageParcel &parcel)
{
    bool status = parcel.ReadInt32(albumId);
    CHECK_AND_RETURN_RET(status, status);
 
    status = parcel.ReadString(filter);
    CHECK_AND_RETURN_RET_LOG(status, false, "filter ReadString failed");
 
    status = DataShare::DataSharePredicates::Unmarshal(predicates, parcel);
    CHECK_AND_RETURN_RET_LOG(status, false, "predicates Unmarshal failed");
 
    return ITypesUtil::Unmarshal(parcel, columns);
}
 
bool AlbumGetSelectedAssetsReqBody::Marshalling(MessageParcel &parcel) const
{
    bool status = parcel.WriteInt32(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
 
    status = parcel.WriteString(filter);
    CHECK_AND_RETURN_RET_LOG(status, false, "filter WriteString failed");
    
    status = DataShare::DataSharePredicates::Marshal(predicates, parcel);
    CHECK_AND_RETURN_RET_LOG(status, false, "predicates Marshal failed");
 
    return ITypesUtil::Marshal(parcel, columns);
}
 
bool AlbumGetSelectedAssetsRespBody::Unmarshalling(MessageParcel &parcel)
{
...
bool AlbumGetSelectedAssetsRespBody::Marshalling(MessageParcel &parcel) const
{
