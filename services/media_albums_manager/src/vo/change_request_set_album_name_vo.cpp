...
#define MLOG_TAG "MediaChangeRequestSetAlbumNameVo"
...
using namespace std;
bool ChangeRequestSetAlbumNameReqBody::Unmarshalling(MessageParcel &parcel)
{
    bool status = parcel.ReadString(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    status = parcel.ReadString(this->albumName);
    CHECK_AND_RETURN_RET(status, status);
...
bool ChangeRequestSetAlbumNameReqBody::Marshalling(MessageParcel &parcel) const
{
    bool status = parcel.WriteString(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    status = parcel.WriteString(this->albumName);
    CHECK_AND_RETURN_RET(status, status);
