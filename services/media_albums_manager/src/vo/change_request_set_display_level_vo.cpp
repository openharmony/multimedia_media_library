...
#define MLOG_TAG "MediaChangeRequestSetDisplayLevelVo"
...
using namespace std;
bool ChangeRequestSetDisplayLevelReqBody::Unmarshalling(MessageParcel &parcel)
{
    bool status = parcel.ReadString(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    status = parcel.ReadInt32(this->displayLevel);
    CHECK_AND_RETURN_RET(status, status);
...
bool ChangeRequestSetDisplayLevelReqBody::Marshalling(MessageParcel &parcel) const
{
    bool status = parcel.WriteString(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    status = parcel.WriteInt32(this->displayLevel);
    CHECK_AND_RETURN_RET(status, status);
