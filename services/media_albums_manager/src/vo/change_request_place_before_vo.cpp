...
bool ChangeRequestPlaceBeforeReqBody::Unmarshalling(MessageParcel &parcel)
{
    bool status = parcel.ReadInt32(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    status = parcel.ReadInt32(this->referenceAlbumId);
    CHECK_AND_RETURN_RET(status, status);
...
bool ChangeRequestPlaceBeforeReqBody::Marshalling(MessageParcel &parcel) const
{
    bool status = parcel.WriteInt32(this->albumId);
    CHECK_AND_RETURN_RET(status, status);
    status = parcel.WriteInt32(this->referenceAlbumId);
    CHECK_AND_RETURN_RET(status, status);
