 ...
 */
#ifndef OHOS_MEDIALIBRARY_PORTRAIT_STRATEGIES_H
#define OHOS_MEDIALIBRARY_PORTRAIT_STRATEGIES_H
...
// 人像相册默认有效策略（直接复用默认）
using PortraitAlbumEffectiveStrategy = AlbumEffectiveStrategyBase;

// 人像计数策略（默认）
using PortraitCountStrategy = CountStrategyBase;

// 人像封面策略
class PortraitCoverStrategy : public CoverStrategyBase {
private:
    bool ShouldRefreshCover(const UpdateAlbumData &oldAlbum, const AnalysisAlbumRefreshInfo &info) override;
};

// 专用封面挑选器
class PortraitCoverPickerStrategy : public CoverPickerStrategyBase {
public:
...
#endif // OHOS_MEDIALIBRARY_PORTRAIT_STRATEGIES_H
