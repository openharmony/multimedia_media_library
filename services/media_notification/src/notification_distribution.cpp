...
int32_t NotificationDistribution::ProcessDbAvailabilityNotifyInfo(const std::string& status, const std::string& reason)
{
    MEDIA_INFO_LOG("ProcessDbAvailabilityNotifyInfo status: %{public}s, reason: %{public}s.",
                   status.c_str(), reason.c_str());
    std::vector<ObserverInfo> observers = findObservers(NotifyUriType::AVAILABILITY_URI);
    CHECK_AND_RETURN_RET_LOG(!observers.empty(), E_ERR, "No observer for AVAILABILITY_URI");
    std::shared_ptr<Parcel> parcel = std::make_shared<Parcel>();
    CHECK_AND_RETURN_RET_LOG(parcel->WriteUint16(static_cast<uint16_t>(NotifyUriType::AVAILABILITY_URI)), E_ERR,
        "Write URI type failed");
    CHECK_AND_RETURN_RET_LOG(parcel->WriteString(status), E_ERR, "Write status failed");
    CHECK_AND_RETURN_RET_LOG(parcel->WriteString(reason), E_ERR, "Write reason failed");
    uintptr_t buf = parcel->GetData();
    auto *uBuf = new (std::nothrow) uint8_t[parcel->GetDataSize()];
    CHECK_AND_RETURN_RET_LOG(uBuf != nullptr, E_ERR, "parcel GetDataSize is null");
    errno_t ret = memcpy_s(uBuf, parcel->GetDataSize(), reinterpret_cast<uint8_t *>(buf), parcel->GetDataSize());
    ...
    serverChangeInfo->data_ = uBuf;
    serverChangeInfo->size_ = parcel->GetDataSize();
    MEDIA_INFO_LOG("ProcessDbAvailabilityNotifyInfo serverChangeInfo->size_ is: %{public}zu", parcel->GetDataSize());
    for (const ObserverInfo& observer : observers) {
        NotificationUtils::SendDbAvailabilityNotification(observer.observer, serverChangeInfo);
    }
    if (serverChangeInfo->data_ != nullptr) {
        delete[] static_cast<uint8_t*>(serverChangeInfo->data_);
        serverChangeInfo->data_ = nullptr;
...
int32_t NotificationDistribution::DistributeDbAvailabilityNotifyInfo(
    const std::vector<std::pair<std::string, std::string>>& notifyInfos)
{
    MEDIA_INFO_LOG("DistributeDbAvailabilityNotifyInfo, size=%zu", notifyInfos.size());
    CHECK_AND_RETURN_RET_INFO_LOG(!notifyInfos.empty(), E_OK, "No tasks to distribute");

    for (const auto& notifyInfo : notifyInfos) {
        const std::string& status = notifyInfo.first;
        const std::string& reason = notifyInfo.second;
        int32_t ret = ProcessDbAvailabilityNotifyInfo(status, reason);
        if (ret != E_OK) {
            MEDIA_ERR_LOG("ProcessDbAvailabilityNotifyInfo failed for status=%s, reason=%s",
                          status.c_str(), reason.c_str());
        }
    }
    return E_OK;
